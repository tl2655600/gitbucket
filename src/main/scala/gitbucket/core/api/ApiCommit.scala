...
object ApiCommit{
  def apply(git: Git, repositoryName: RepositoryName, commit: CommitInfo, maxFetchSize: Long): ApiCommit = {
    val diffs = JGitUtil.getDiffs(git, commit.id, false, maxFetchSize)
    ApiCommit(
      id        = commit.id,
